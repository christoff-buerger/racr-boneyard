		...
		then
			...
			do
				...
				then
					...
					case $system in
					Larceny)
						measure larceny --r6rs --path "$old_pwd/cjava-racr/larceny-bin:$old_pwd/../racr/racr/larceny-bin" \
							--program $old_pwd/run-cjava-compiler.scm -- $mode $recipe $targets $sources;;
					Petite)
						measure petite --libdirs "$old_pwd:$old_pwd/../racr" \
							--program $old_pwd/run-cjava-compiler.scm $mode $recipe $targets $sources;;
					Racket)
						measure plt-r6rs ++path "$old_pwd/cjava-racr/racket-bin" ++path "$old_pwd/../racr/racr/racket-bin" \
